...
public class LocustCommand implements CommandExecutor {
    private bountyHunter plugin;

    public LocustCommand(bountyHunter plugin){
        this.plugin = plugin;
    }
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(sender instanceof Player){
            ...
            for (int i = 0; i <= bountyWrapperList.size() - 1; i++) {
                ...
                if (X || Y || Z) {
                    if (bountyWrapperList.get(i).getType().equals("locust")) {
                        ...
                        if (isInList) {
                            ...
                            if (foundWrapper.getTeam().equals("locust")) {
                                player.sendMessage(
                                        plugin
                                                .getConfigHelper()
                                                .getStringMessage(
